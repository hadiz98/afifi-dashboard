...
import { ChevronDown, PipetteIcon } from "lucide-react"
...
function ColorPickerArea(props: ColorPickerAreaProps) {
  const { asChild: _asChild, className, ref, ...areaProps } = props
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute size-3 -translate-x-1/2 rtl:translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white shadow-sm"
        style={{
          left: `${hsv?.s ?? 0}%`,
          top: `${100 - (hsv?.v ?? 0)}%`,
        }}
...
function ColorPickerHueSlider(props: ColorPickerHueSliderProps) {
  ...
  return (
    ...
    >
      ...
      >
        <Slider.Track className="relative h-3 w-full grow overflow-hidden rounded-full bg-[linear-gradient(to_right,#ff0000_0%,#ffff00_16.66%,#00ff00_33.33%,#00ffff_50%,#0000ff_66.66%,#ff00ff_83.33%,#ff0000_100%)]">
          <Slider.Indicator className="rounded-full bg-white/15" />
          <Slider.Thumb
            aria-label="Hue"
...
function ColorPickerSwatch(props: ColorPickerSwatchProps) {
  const { asChild: _asChild, className, ...swatchProps } = props
...
const FORMAT_SELECT_ITEMS = colorFormats.map((format) => ({
  value: format,
  ...
  return (
    ...
    >
      <Select.Trigger
        ...
        className={cn(
          "flex h-8 min-w-[4.5rem] shrink-0 items-center justify-between gap-1 rounded-md border border-input bg-background px-2 text-xs outline-none select-none hover:bg-muted/60 focus-visible:ring-2 focus-visible:ring-ring/40",
          className
        )}
      >
        <Select.Value className="truncate data-placeholder:text-muted-foreground" />
        <Select.Icon className="flex opacity-60">
          <ChevronDown className="size-3.5" />
        </Select.Icon>
      </Select.Trigger>
      <Select.Portal>
        <Select.Positioner className="z-[100]" sideOffset={4}>
          <Select.Popup className="max-h-[min(var(--available-height),280px)] origin-[var(--transform-anchor)] rounded-md border bg-popover p-1 text-popover-foreground shadow-md ring-1 ring-foreground/10">
            <Select.List className="max-h-[inherit] scroll-py-1 overflow-y-auto">
              {FORMAT_SELECT_ITEMS.map((item) => (
                <Select.Item
                  key={item.value}
                  value={item.value}
                  className="flex cursor-default items-center rounded-sm px-2 py-1.5 text-xs outline-none select-none data-highlighted:bg-muted"
                >
