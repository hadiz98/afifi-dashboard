...
import {
  HorsePedigreeDialog,
  type PedigreeRelKey,
} from "@/components/pages/horse-pedigree-dialog";
...
function translationsFromItem(item: HorseDetails): TranslationsForm {
  ...
  for (const loc of ["en", "ar"] as const) {
    if (!base[loc].color.trim() && rootColor) base[loc] = { ...base[loc], color: rootColor };
    if (!base[loc].breeder.trim() && rootBreeder) base[loc] = { ...base[loc], breeder: rootBreeder };
  }
  return base;
}
...
function clonePedigree(p: HorsePedigree | null | undefined): HorsePedigree {
  ...
  for (const k of ["father", "mother"] as const) {
    const r = p[k];
    if (r) copy[k] = { id: r.id, name: r.name, birthDate: r.birthDate, color: r.color };
  }
  return copy;
}
...
function pedigreeHasContent(p: HorsePedigree | null | undefined): boolean {
  ...
  for (const k of ["father", "mother"] as const) {
    const r = p[k];
    if (r && (String(r.id ?? "").trim() || String(r.name ?? "").trim() || String(r.birthDate ?? "").trim() || String(r.color ?? "").trim())) {
      return true;
    }
  }
  return false;
...
const translationDialogRequiredKeys = new Set<keyof TranslationLocaleFields>([
  "name", "shortBio", "description", "tags",
]);
...
function PedigreeReadonlyCards({
  ...
  labels: { father: string; mother: string; birthDate: string; color: string };
}) {
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-2">
      {rows.map(({ key, title: ttl, rel }) => {
        if (!rel || (!String(rel.id ?? "").trim() && !String(rel.name ?? "").trim() && !String(rel.birthDate ?? "").trim() && !String(rel.color ?? "").trim())) {
          return null;
        }
        return (
          <div key={key} className="rounded-xl border border-border/60 bg-background px-3 py-2.5">
            ...
            {rel.name ? (
              rel.id ? (
                <Link href={`/horses/${rel.id}`} className="mt-1 inline-block text-sm font-medium text-foreground underline-offset-2 hover:underline">
                  {rel.name}
                </Link>
              ) : (
                <p className="mt-1 text-sm font-medium text-foreground">{rel.name}</p>
              )
            ) : null}
            {!rel.name && rel.id ? <p className="mt-1 text-xs font-medium text-foreground">{rel.id}</p> : null}
...
export function HorseDetailsPanel({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItem(await fetchHorseById(id));
    } catch (e) {
  ...
  async function onSubmitPedigree(payload: HorsePedigreeUpdatePayload) {
    ...
    setSubmitting(true);
    try {
      await updateHorsePedigree(id, payload);
      toast.success(t("saveSuccess"));
      await load();
      setPedigreeEditOpen(false);
    } catch (e) {
  ...
  // ─── Profile / cover / translation persistence ────────────────────────────
  ...
  const persistHorseUpdate = useCallback(
    async (patch: {
      ...
      try {
        ...
        if (patch.translationPatch) {
          const { locale: loc, fields } = patch.translationPatch;
          fd.append("translations", JSON.stringify({ [loc]: buildLocaleTranslationPayload(fields) }));
        }
  ...
  async function onUploadMedia() {
    ...
    try {
      ...
      toast.success(created.length === 1 ? tMedia("uploadSuccess") : tMedia("uploadSuccessMany", { count: created.length }));
  ...
  async function onReplaceMediaFile() {
    ...
    if (mediaReplaceFile.size > HORSE_GALLERY_MAX_FILE_BYTES) { toast.error(tMedia("fileTooLarge")); return; }
    if (!isAllowedHorseGalleryFile(mediaReplaceFile)) { toast.error(tMedia("invalidFileType")); return; }
    setSubmitting(true);
    try {
  ...
  // ─── Profile / cover / translation handlers ───────────────────────────────
  ...
  const translationEnSaveDisabled = submitting || !translationEnForm || !hasLocaleRequired(translationEnForm);
  const translationArSaveDisabled = submitting || !translationArForm || !hasLocaleRequired(translationArForm);
  const activeTranslationSaveDisabled = translationTab === "en" ? translationEnSaveDisabled : translationArSaveDisabled;
  ...
  async function onSaveCover() {
    if (!coverEditFile) { toast.error(t("coverRequired")); return; }
    const ok = await persistHorseUpdate({ coverFile: coverEditFile });
    if (ok) { setCoverEditOpen(false); setCoverEditFile(null); }
  }

  async function onSaveTranslationEn() {
    if (!translationEnForm) return;
    const ok = await persistHorseUpdate({ translationPatch: { locale: "en", fields: translationEnForm } });
    if (ok) setTranslationEditOpen(false);
  }

  async function onSaveTranslationAr() {
    if (!translationArForm) return;
    const ok = await persistHorseUpdate({ translationPatch: { locale: "ar", fields: translationArForm } });
    if (ok) setTranslationEditOpen(false);
  }

  async function onSaveActiveTranslation() {
    if (translationTab === "en") await onSaveTranslationEn();
    else await onSaveTranslationAr();
  }
  ...
  const displayPedigree = useMemo(() => {
    if (!item) return null;
    const base = clonePedigree(item.pedigree);
    if (item.sireId || item.sire?.name) {
      base.father = {
        ...(base.father ?? {}),
        id: item.sireId ?? base.father?.id ?? null,
        name: item.sire?.name ?? base.father?.name ?? null,
      };
    }
    if (item.damId || item.dam?.name) {
      base.mother = {
        ...(base.mother ?? {}),
        id: item.damId ?? base.mother?.id ?? null,
        name: item.dam?.name ?? base.mother?.name ?? null,
      };
    }
    return base;
  }, [item]);
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4 px-3 py-5 sm:px-6 sm:py-8">
      ...
      {/* Hero loading / not found / content */}
      {loading ? (
        <>
          <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
        ...
        <>
            ...
            <div className="space-y-4">
              <div className="min-w-0">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-1.5">
                  <Badge variant="secondary" className="rounded-full font-normal">{categoryLabel(tList, item.category)}</Badge>
          ...
          <Section
            ...
            actions={
              <>
                ...
                {mediaReorderMode ? (
                  <>
                    <Button type="button" variant="outline" size="sm" className="h-8 text-xs" onClick={() => setMediaReorderMode(false)}>{tMedia("done")}</Button>
                    <Button type="button" size="sm" className="h-8 text-xs" disabled={submitting || mediaRows.length < 2} onClick={() => void persistMediaOrder()}>{tMedia("saveOrder")}</Button>
                  </>
                ) : (
                  <>
                    {mediaRows.length >= 2 && (
                      <Button type="button" variant="outline" size="sm" className="h-8 text-xs" disabled={submitting} onClick={() => setMediaReorderMode(true)}>{tMedia("reorder")}</Button>
                    )}
                    <Button type="button" size="sm" className="h-8 gap-1.5 text-xs" disabled={submitting} onClick={() => setMediaUploadOpen(true)}>
              ...
              <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 md:grid-cols-4">
                {mediaRows.map((m, idx) => (
                  <div key={m.id} className="group relative overflow-hidden rounded-xl border border-border/60 bg-muted">
                    <button type="button" className="relative block aspect-[4/3] w-full overflow-hidden" onClick={() => openViewer(idx)}>
                    ...
                    {mediaReorderMode ? (
                      <div className="flex items-center justify-between gap-1 border-t border-border/60 bg-background px-2 py-1.5">
                        <Button type="button" size="icon" variant="outline" className="h-6 w-6" disabled={idx === 0 || submitting} onClick={() => moveMedia(idx, -1)}><ArrowUp className="size-3" /></Button>
                        <span className="text-[10px] text-muted-foreground">{idx * 10}</span>
                        <Button type="button" size="icon" variant="outline" className="h-6 w-6" disabled={idx === mediaRows.length - 1 || submitting} onClick={() => moveMedia(idx, 1)}><ArrowDown className="size-3" /></Button>
                      </div>
                    ) : (
          ...
          <Section
            ...
            actions={
              <>
                <Button type="button" variant="ghost" size="sm" className="h-8 w-8 p-0" disabled={awardsLoading || submitting} onClick={() => void loadAwards()}>
                  <RefreshCw className={cn("size-3.5", awardsLoading && "animate-spin")} />
                </Button>
                <Button type="button" size="sm" className="h-8 gap-1.5 text-xs" disabled={submitting} onClick={openAwardCreate}>
                ...
                {awardRows.map((a) => (
                  <div key={a.id} className="group flex items-start justify-between gap-3 rounded-xl border border-border/60 bg-background px-4 py-3 transition-colors hover:bg-muted/30">
                    <div className="flex items-start gap-3 min-w-0">
                      <div className="flex size-8 shrink-0 items-center justify-center rounded-lg border border-border/60 bg-muted/50 text-xs font-bold text-muted-foreground">{a.year}</div>
                      <div className="min-w-0">
                        ...
                        {a.externalLink ? (
                          <a href={a.externalLink} target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                      <Button type="button" size="icon" variant="ghost" className="h-7 w-7" onClick={() => openAwardEdit(a)} disabled={submitting}><Pencil className="size-3.5" /></Button>
                      <Button type="button" size="icon" variant="ghost" className="h-7 w-7 text-destructive hover:text-destructive hover:bg-destructive/10" disabled={submitting}
          ...
          <Section
            ...
            actions={
              <Button
                type="button" variant="outline" size="sm" className="h-8 gap-1.5 text-xs"
                disabled={submitting}
                onClick={() => setPedigreeEditOpen(true)}
              >
                <Pencil className="size-3" /><span className="hidden sm:inline">{t("edit")}</span>
              </Button>
            }
          >
            {!pedigreeHasContent(displayPedigree) ? (
              <p className="text-sm text-muted-foreground">{t("pedigreeEmpty")}</p>
            ) : (
              <PedigreeReadonlyCards
                pedigree={displayPedigree}
                labels={{
              ...
              <div className="flex gap-1">
                {(["en", "ar"] as const).map((loc) => (
                  <span key={loc} className={cn(
                    ...
                    item.translations.some((x) => x.locale === loc) ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                  )}>{loc}</span>
                ))}
              </div>
            }
            actions={
              <Button type="button" variant="outline" size="sm" className="h-8 gap-1.5 text-xs" disabled={submitting}
                onClick={() => {
          ...
          >
            <Tabs value={translationTab} onValueChange={(v) => setTranslationTab((v as HorseLocale) === "ar" ? "ar" : "en")}>
              <TabsList className="w-full justify-between sm:w-auto sm:justify-start">
                <TabsTrigger value="en" className="flex-1 sm:flex-none">{t("langEn")}</TabsTrigger>
                <TabsTrigger value="ar" className="flex-1 sm:flex-none">{t("langAr")}</TabsTrigger>
              </TabsList>
              {(["en", "ar"] as const).map((loc) => {
      ...
      {/* Edit profile */}
      <Dialog open={profileEditOpen} onOpenChange={(open) => { setProfileEditOpen(open); if (!open) setProfileForm(null); else if (item) setProfileForm(profileFormFromItem(item)); }}>
        <DialogContent className="w-[calc(100vw-1.5rem)] max-h-[95dvh] overflow-hidden flex flex-col sm:w-auto sm:max-w-[760px]">
              ...
              <div className="grid gap-4 py-4 pr-1">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="grid gap-1.5">
                    <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("fieldSlug")}<RequiredStar /></Label>
                    <Input value={profileForm.slug} onChange={(e) => setProfileForm((s) => s ? { ...s, slug: e.target.value } : s)} className="h-9" />
                  </div>
                  <div className="grid gap-1.5">
                    <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("fieldCategory")}<RequiredStar /></Label>
                    <select className="h-9 w-full rounded-lg border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      value={profileForm.category}
                      onChange={(e) => setProfileForm((s) => s ? { ...s, category: e.target.value as HorseCategory } : s)}>
                  ...
                  <div className="grid gap-1.5">
                    ...
                    <Input type="date" value={profileForm.birthDate} className="h-9" onChange={(e) => setProfileForm((s) => s ? { ...s, birthDate: e.target.value } : s)} />
                  </div>
                  <div className="grid gap-1.5">
                    <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("fieldHeightCm")}</Label>
                    <Input inputMode="numeric" value={profileForm.heightCm} className="h-9" onChange={(e) => setProfileForm((s) => s ? { ...s, heightCm: e.target.value } : s)} />
                  </div>
                  <div className="grid gap-1.5 sm:col-span-2">
                    <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("fieldOwner")}</Label>
                    <Input value={profileForm.owner} className="h-9" onChange={(e) => setProfileForm((s) => s ? { ...s, owner: e.target.value } : s)} />
                  </div>
                  <div className="grid gap-1.5 sm:col-span-2">
                    <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("fieldNotes")}</Label>
                    <Textarea value={profileForm.notes} rows={3} className="resize-none text-sm" onChange={(e) => setProfileForm((s) => s ? { ...s, notes: e.target.value } : s)} />
                  </div>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
                  <div>
                    <Label className="text-sm font-medium">{t("fieldIsActive")}</Label>
                    <p className="text-xs text-muted-foreground">{profileForm.isActive ? tList("active") : tList("inactive")}</p>
                  </div>
                  <Switch checked={profileForm.isActive} onCheckedChange={(v) => setProfileForm((s) => s ? { ...s, isActive: v } : s)} />
                </div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
                    <div>
                  ...
                  <div className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{profileForm.isHeritage ? t("yes") : t("no")}</p>
                    </div>
                    <Switch checked={profileForm.isHeritage} onCheckedChange={(v) => setProfileForm((s) => s ? { ...s, isHeritage: v } : s)} />
                  </div>
                </div>
              </div>
            ) : null}
          ...
          <DialogFooter className="shrink-0 gap-2 pt-2">
            <Button type="button" variant="outline" disabled={submitting} onClick={() => setProfileEditOpen(false)}>{t("cancel")}</Button>
            <Button type="button" disabled={profileSaveDisabled} onClick={() => void onSaveProfile()} className="gap-1.5 min-w-24">
      ...
      {/* Edit cover */}
      <Dialog open={coverEditOpen} onOpenChange={(open) => { setCoverEditOpen(open); if (!open) setCoverEditFile(null); }}>
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base font-semibold">
              <div className="flex size-7 items-center justify-center rounded-lg border bg-muted"><ImageIcon className="size-3.5 text-muted-foreground" /></div>
          ...
          <div className="grid gap-3 py-2">
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("fieldCoverImage")}<RequiredStar /></Label>
            <label htmlFor="horse-cover-only" className="group flex cursor-pointer flex-col items-center gap-3 rounded-xl border border-dashed border-border/70 bg-muted/20 py-8 text-center transition-colors hover:border-border hover:bg-muted/30">
              ...
              {coverEditFile ? (
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{(coverEditFile.size / 1024).toFixed(1)} KB · {t("clickToReplace")}</p>
                </div>
              ) : (
                <div>
              ...
              <Input id="horse-cover-only" type="file" accept="image/*" className="sr-only" onChange={(e) => setCoverEditFile(e.target.files?.[0] ?? null)} />
            </label>
          </div>
          <Separator />
          <DialogFooter className="gap-2">
            <Button type="button" variant="outline" disabled={submitting} onClick={() => setCoverEditOpen(false)}>{t("cancel")}</Button>
            <Button type="button" disabled={submitting || !coverEditFile} onClick={() => void onSaveCover()} className="gap-1.5">
      ...
      <HorsePedigreeDialog
        ...
        onOpenChange={setPedigreeEditOpen}
        onSubmit={onSubmitPedigree}
        t={t}
      />

      {/* Edit translation */}
      <Dialog open={translationEditOpen} onOpenChange={(open) => {
        setTranslationEditOpen(open);
        if (!open) { setTranslationEnForm(null); setTranslationArForm(null); }
        else if (item) { const all = translationsFromItem(item); setTranslationEnForm({ ...all.en }); setTranslationArForm({ ...all.ar }); }
      }}>
        <DialogContent className="w-[calc(100vw-1.5rem)] max-h-[92dvh] overflow-hidden flex flex-col sm:w-auto sm:max-w-[640px]">
          <DialogHeader className="shrink-0">
            <DialogTitle className="flex items-center gap-2 text-base font-semibold">
              <div className="flex size-7 items-center justify-center rounded-lg border bg-muted"><Globe className="size-3.5 text-muted-foreground" /></div>
          ...
          <Separator />
          <Tabs value={translationTab} onValueChange={(v) => setTranslationTab(v === "ar" ? "ar" : "en")} className="min-h-0 flex flex-1 flex-col">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto py-2 pr-1">
              <TabsContent value="en">
                {translationEnForm ? (
                  <TranslationFieldsEditor dir="ltr" value={translationEnForm}
                    onField={(key, v) => setTranslationEnForm((s) => s ? { ...s, [key]: v } : s)} t={t} />
                ) : null}
              </TabsContent>
              <TabsContent value="ar">
                {translationArForm ? (
                  <TranslationFieldsEditor dir="rtl" value={translationArForm}
                    onField={(key, v) => setTranslationArForm((s) => s ? { ...s, [key]: v } : s)} t={t} />
                ) : null}
              </TabsContent>
            </div>
          </Tabs>
          ...
          <DialogFooter className="shrink-0 gap-2">
            <Button type="button" variant="outline" disabled={submitting} onClick={() => setTranslationEditOpen(false)}>{t("cancel")}</Button>
            <Button type="button" disabled={activeTranslationSaveDisabled} onClick={() => void onSaveActiveTranslation()} className="gap-1.5">
              {submitting ? <RefreshCw className="size-3.5 animate-spin" /> : <Pencil className="size-3.5" />}
              {t("save")}
            </Button>
          </DialogFooter>
        ...
        <DialogContent className="max-w-[96vw] gap-3 p-3 sm:max-w-[860px]">
          <div className="relative overflow-hidden rounded-xl border border-border/60 bg-muted">
            {mediaRows[viewerIndex] ? (
              <img src={mediaRows[viewerIndex]!.url} alt={mediaRows[viewerIndex]!.caption ?? ""} className="max-h-[75dvh] w-full object-contain" decoding="async" />
            ) : (
              <div className="flex h-64 items-center justify-center"><ImageIcon className="size-8 text-muted-foreground/30" /></div>
            )}
            <div className="absolute left-2 top-1/2 -translate-y-1/2">
              <Button type="button" size="icon" variant="secondary" className="h-8 w-8 shadow-md" disabled={viewerIndex <= 0} onClick={() => setViewerIndex((i) => Math.max(0, i - 1))}>
                <ChevronLeft className="size-4 rtl:rotate-180" />
              </Button>
            </div>
            <div className="absolute right-2 top-1/2 -translate-y-1/2">
              <Button type="button" size="icon" variant="secondary" className="h-8 w-8 shadow-md" disabled={viewerIndex >= mediaRows.length - 1} onClick={() => setViewerIndex((i) => Math.min(mediaRows.length - 1, i + 1))}>
                <ChevronRight className="size-4 rtl:rotate-180" />
              </Button>
            </div>
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2">
              <span className="rounded-full bg-black/60 px-2.5 py-1 text-xs font-medium text-white backdrop-blur">{viewerIndex + 1} / {mediaRows.length}</span>
            </div>
          </div>
          {mediaRows[viewerIndex]?.caption && <p className="px-1 text-sm text-muted-foreground">{mediaRows[viewerIndex]!.caption}</p>}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={mediaUploadOpen} onOpenChange={onMediaUploadDialogOpenChange}>
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="grid gap-3 py-1">
            <div className="grid gap-1.5">
              <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{tMedia("files")}<RequiredStar /></Label>
              <input ref={mediaUploadInputRef} type="file" accept={HORSE_GALLERY_ACCEPT} multiple className="sr-only" onChange={onMediaUploadPick} />
              <button type="button"
                ...
                className={cn("flex min-h-[120px] w-full flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed px-4 py-6 text-center text-xs transition-colors",
                  mediaUploadDropOver ? "border-primary/60 bg-primary/5" : "border-border/80 bg-muted/20 hover:border-border")}>
              ...
              {mediaUploadError ? <p className="text-xs text-destructive">{mediaUploadError}</p> : null}
                  ...
                  {mediaUploadFiles.map((f, idx) => (
                    <li key={`${f.name}-${f.size}-${idx}`} className="flex items-center gap-2 text-xs">
                      ...
                      <Button type="button" variant="ghost" size="icon" className="size-7 shrink-0"
                        onClick={(ev) => { ev.stopPropagation(); setMediaUploadFiles((prev) => prev.filter((_, i) => i !== idx)); setMediaUploadError(null); }}>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button type="button" disabled={submitting || mediaUploadFiles.length === 0} onClick={() => void onUploadMedia()} className="gap-1.5">
              {submitting ? <RefreshCw className="size-3.5 animate-spin" /> : <Upload className="size-3.5" />}
              {tMedia("upload")}
            </Button>
          </DialogFooter>
      ...
      <Dialog open={mediaReplaceOpen} onOpenChange={setMediaReplaceOpen}>
        <DialogContent className="sm:max-w-[480px]">
          ...
          <div className="grid gap-3 py-1">
            <div className="grid gap-1.5">
              <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{tMedia("file")}<RequiredStar /></Label>
              <Input type="file" accept={HORSE_GALLERY_ACCEPT} className="h-9" onChange={(e) => setMediaReplaceFile(e.target.files?.[0] ?? null)} />
            </div>
          </div>
          <Separator />
          <DialogFooter className="gap-2">
        ...
        <DialogContent className="sm:max-w-[560px]">
          ...
          <div className="grid gap-3 py-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{tAwards("fieldYear")}<RequiredStar /></Label>
                <Input inputMode="numeric" value={awardForm.year} placeholder="2026" className="h-9" onChange={(e) => setAwardForm((s) => ({ ...s, year: e.target.value }))} />
              </div>
              <div className="grid gap-1.5">
                <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{tAwards("fieldEventName")}<RequiredStar /></Label>
                <Input value={awardForm.eventName} className="h-9" onChange={(e) => setAwardForm((s) => ({ ...s, eventName: e.target.value }))} />
              </div>
            </div>
            <div className="grid gap-1.5">
              <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{tAwards("fieldTitle")}<RequiredStar /></Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="grid gap-1.5">
                ...
                <Input value={awardForm.placing} className="h-9" onChange={(e) => setAwardForm((s) => ({ ...s, location: e.target.value }))} />
              </div>
            </div>
            <div className="grid gap-1.5">
              <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{tAwards("fieldExternalLink")}</Label>
              <Input type="url" value={awardForm.externalLink} placeholder={tAwards("fieldExternalLinkPlaceholder")} className="h-9" onChange={(e) => setAwardForm((s) => ({ ...s, externalLink: e.target.value }))} />
            </div>
            <div className="grid gap-1.5">
              <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{tAwards("fieldNotes")}</Label>
              <Textarea value={awardForm.notes} rows={3} className="resize-none text-sm" onChange={(e) => setAwardForm((s) => ({ ...s, notes: e.target.value }))} />
            </div>
          </div>
          <Separator />
          <DialogFooter className="gap-2">
